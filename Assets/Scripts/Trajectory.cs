...
public class Trajectory : MonoBehaviour
{
	...
	public void UpdateDots(Vector3 ballPos, Vector3 forceApplied, float dragCoeff) {
		float timeStamp = dotSpacing;
		bool hideDot = false;
		foreach (Transform dot in _dotsList) {
			Vector3 dotPos;
            // Calculating positions of dots acc to basic physic formulas
            time =  (Mathf.Round(Time.deltaTime * 1000)) / 1000.0f;
            forceApplied = forceApplied * (1 - dragCoeff * time);//Time.deltaTime);
