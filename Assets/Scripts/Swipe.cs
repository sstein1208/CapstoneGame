...
public class Swipe : MonoBehaviour 
{
    ...
    [SerializeField] protected float dragCoeff;
	...
	private Vector3 _forcevector, _dragvector;		// Force vectors to apply ball
	...
	private void ControlSwipe() {
        ...
        if (Input.GetMouseButton(0)) {
            ...
            _dragvector = new Vector3(_forcevector.x, 0f, _forcevector.z);

			// Updating positions of dots acc to Force vector starting with ball position
			trajectory.UpdateDots(transform.position, _forcevector, dragCoeff);
		}
		...
		if (Input.GetMouseButtonUp(0)) {
			if (_ball) {
				...
				_ball.Push(_forcevector, _dragvector * -dragCoeff);
				_ball = null;
